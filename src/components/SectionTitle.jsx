...
function SectionTitle({ eyebrow, title, subtitle, scriptAccent }) {
  return (
    ...
    >
      {eyebrow ? <p className="font-accent text-sm font-bold uppercase tracking-[0.16em] text-accent md:text-base">{eyebrow}</p> : null}
      ...
      {scriptAccent ? (
        <p className="font-script mt-3 text-3xl font-bold leading-none text-accent md:text-4xl">{scriptAccent}</p>
      ) : null}
      {subtitle ? (
